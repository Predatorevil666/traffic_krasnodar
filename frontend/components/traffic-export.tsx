...
export function TrafficExport({ onClose }: TrafficExportProps) {
	...
	const handleExport = async () => {
		try {
			const exportData = {
				type: activeTab,
				startDate: activeTab === "range" ? startDate?.toISOString() : null,
				endDate: activeTab === "range" ? endDate?.toISOString() : null,
				singleDate: activeTab === "single" ? singleDate?.toISOString() : null
			}
			...
			let filename
			if (activeTab === "range") {
				filename = `traffic-data-${startDate?.toISOString().split("T")[0]}-to-${endDate?.toISOString().split("T")[0]}.xlsx`
			} else {
				filename = `traffic-data-${singleDate?.toISOString().split("T")[0]}.xlsx`
			}
